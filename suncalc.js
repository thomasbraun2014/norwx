/*
 ...
 BSD-2-Clause License, (c) 2014, Vladimir Agafonkin
 Wrapped as browser global for NorWX PWA
*/
  ...
  return {
    ...
    getTimes: function(date, lat, lng, height) {
      ...
      for (var i = 0; i < times.length; i++) {
        var time = times[i], h0 = (time[0] + dh) * rad,
    ...
    getMoonPosition: function(date, lat, lng) {
      ...
      return { azimuth: azimuth(H, phi, c.dec), altitude: h + astroRefraction(h), distance: c.dist, parallacticAngle: pa };
    },
    ...
    getMoonTimes: function(date, lat, lng, inUTC) {
      ...
      var hc = 0.133 * rad, h0 = this.getMoonPosition(t, lat, lng).altitude - hc, rise, set, ye;
      for (var i = 1; i <= 24; i += 2) {
        var h1 = this.getMoonPosition(hoursLater(t, i), lat, lng).altitude - hc;
        var h2 = this.getMoonPosition(hoursLater(t, i + 1), lat, lng).altitude - hc;
        var a = (h0 + h2) / 2 - h1, b = (h2 - h0) / 2, xe = -b / (2 * a),
            dd = b * b - 4 * a * h1, roots = 0, x1 = 0, x2 = 0;
        ye = (a * xe + b) * xe + h1;
        if (dd >= 0) {
          var dx = Math.sqrt(dd) / (Math.abs(a) * 2);
          x1 = xe - dx; x2 = xe + dx;
